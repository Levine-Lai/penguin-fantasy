...
- 所有玩家统一按以下规则排名：生命值降序、队长总分降序、历史各轮队长选择率累计升序；前三项完全相同时，`SSU - Sakai Moka` 与 `企鹅`优先，再按名称首字母排序。
- 第一阶段积分排行榜下方固定显示当轮“队长选择率”榜单，按选择人数降序展示队长名字、基础分、选择人数与选择率；表头与每行四列全部水平、垂直居中，不得为此增加筛选或分页按钮。
...
- 队长抓取完成状态必须按当前名单的 `entryId` 逐一核对，不得仅比较数组长度；名单增减后只补抓当前仍缺失的玩家，离开联赛的旧玩家不得继续计入选择率分母。
- 抓取任务优先完成最新已到捕获时间的 GW；最新轮完成后，必须继续回补因 FPL 故障遗留的旧 GW，避免历史出现永久断层。
- 旧 GW 的队长选择补齐后，下一次 07:30 发布必须同时补建缺失的历史快照，并在结束后保持状态指向当前最新 GW。
...
- 队长选择记录标题右侧始终显示与右上角“当前试炼”相同的当前 GW，不得保留“见习者集结”静态文案。
- 首屏使用内置官方 DDL 日程直接计算当前 GW，API 返回后再校正，刷新和新开页面不得先闪现“见习者集结”。
